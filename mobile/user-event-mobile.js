...
//*************************************************************************************************************************************************
	...
	function GUTAUS () {
		...
		this.creditnote_pay = new CREDITNOTE_PAY();
		...
		this.pay= function() {
			...
			url="./model/creditnote-pay.php";

			$.ajax({
				type: "post",
				url: url,
				data: {creditnote : gutaus.creditnote_chosen.name,
					   receiverId : gutaus.member_chosen.id, // noch erstellen
					   receiver : gutaus.member_chosen.member, // name email or mobilenumber
					   amount : gutaus.creditnote_chosen.amount, // noch erstellen
					   purpose : gutaus.creditnote_chosen.pay_purpose, // noch erstellen
					   typeOfTransfer : gutaus.creditnote_chosen.pay_type_of_transfer // noch erstellen
					  },                   
				async: true,
				beforeSend: function() {
				...
				success: function (str_pay) {					
					...
					if (typeof pay.login === 'undefined') { // user is logged in
						if (typeof pay.errors != 'undefined') {
							...
							gutaus.creditnote_chosen.account_balance=gutaus.creditnote_chosen.account_balance-gutaus.creditnote_pay.amount;
							...
							gutaus.creditnote_chosen.total_turnover=gutaus.creditnote_chosen.total_turnover-gutaus.creditnote_pay.amount;
		...
		function USER_DATA () {
			...
			this.validate_user_input=function(){
				...
				if (activePage=="pay-email") {
					gutaus.creditnote_pay.receiver=$("#input-email-receiver").val();
					gutaus.member_chosen.receiver=$("#input-email-receiver").val();
					gutaus.member_chosen.receiver=gutaus.member_chosen.receiver.trim();
					if(gutaus.member_chosen.validateEmail(gutaus.member_chosen.receiver)) {
						gutaus.member_chosen.get_member_info_of_email(gutaus.member_chosen.receiver);
					}
					else {
						alert('Fehler: '+gutaus.member_chosen.receiver+' ist keine E-Mail-Adresse!');
						$("#input-email-receiver").focus();	
					}															
				} 
				else if (activePage=="pay-mobile") {
					gutaus.creditnote_pay.receiver=$("#input-email-receiver").val();
					gutaus.member_chosen.receiver=$("#input-mobile-receiver").val()
					gutaus.member_chosen.receiver=gutaus.member_chosen.receiver.trim();
					if(gutaus.member_chosen.validateMobile(gutaus.member_chosen.receiver)) {
						gutaus.member_chosen.get_member_info_of_mobile(gutaus.member_chosen.receiver);
						gutaus.member_chosen.receiver=$("#input-mobile-receiver").val();
					}
					else {
						alert('Fehler: '+gutaus.member_chosen.receiver+' ist keine Handynummer! Bitte geben Sie nur Zahlen ein und nicht mehr als 15 Ziffern!' );
					...
					gutaus.creditnote_pay.amount=runde(Math.abs($("#input-pay-amount").val()),gutaus.creditnote_chosen.decimal_digits);
					$("#input-pay-amount").val(gutaus.creditnote_pay.amount);
					if (gutaus.creditnote_pay.amount<=gutaus.creditnote_chosen.get_max_buying_power()) {
					...
					gutaus.creditnote_pay.purpose=$("#input-pay-purpose").val();
					gutaus.creditnote_pay.creditnote=gutaus.creditnote_chosen.name;
					gutaus.creditnote_pay.receiver=gutaus.member_chosen.receiver;
					gutaus.creditnote_pay.pay();
				}
		...
		//CREDITNOTE_PAY
		function CREDITNOTE_PAY () {
		
			//PROPERTIES
			this.creditnote;  // = gutaus.creditnote_chosen.name 
			this.receiver_id;
			this.receiver; // = gutaus.creditnote_chosen.receiver
			this.amount;
			this.purpose;
			this.type_of_transfer;
			...
			this.pay= function() {
				...
				$.ajax({
					...
					data: {creditnote : this.creditnote,
						   ...
						   typeOfTransfer : this.type_of_transfer
						  },                   
					async: true,
					beforeSend: function() {
						// This callback function will trigger before data is sent
		...
		function MEMBERS () {
			...
			this.get= function() {
				...
				if (typeof(url)!='undefined') { // members-list is not loaded so start loading from DB
					$.ajax({
						type: "post",
						url: url,
				...
				else { // member-list to get is loaded only show datas
					gutaus.members.show();
				}			
			...
			this.show= function() {	
				...
				$.each(members,function(i,member){ // fill members-list
					$("#list-members").append("<li><a href='#member' "
													+"onclick=gutaus.member_chosen.get_member_info_of_id('"
													+member.id
													+"')>"
													+member.name
													+"</a></li>");
		...
		//PROPERTIES
			...
			this.receiver; // a Memeber Name , a Email address or a mobile number
			this.member;
		...
		//METHODS
			...
			// get methods  
			this.get_member_info_of_id= function(id) {
				gutaus.creditnote_pay.receiver_id=id;
				var url;
				...
				$.ajax({
					...
					success: function (str_member_out_of_DB) {					
						...
						if (typeof member_out_of_DB.login === 'undefined') {
							...
							gutaus.member_chosen.receiver=gutaus.member_chosen.member.name;
			...
			this.get_member_info_of_email= function(email) {
				var url;
						...
						if (typeof member_out_of_DB.login === 'undefined') {
							...
							if (gutaus.member_chosen.member.name!="") { //member of email was found in DB
								if(gutaus.member_chosen.member.id!=gutaus.user_data.id) {
									gutaus.creditnote_pay.receiver_id=gutaus.member_chosen.member.id;
									alert('Das Mitglied '
			...
			this.get_member_info_of_mobile= function(mobile) {
				var url;
			...
			this.show= function() {
				...
				//$(".creditnote_account_balance").html(this.account_balance);
				//$(".creditnote_credit_limit").html(this.credit_limit);
				//$(".creditnote_acceptance_limit").html(this.acceptance_limit);
				//$(".creditnote_total_turnover").html(this.total_turnover);						
			}
...
// Publischer +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++			
			$("#btn-creditnotes-pay_to_publisher").on("click", function(){
				...
				gutaus.creditnote_pay.type_of_transfer="member";
				gutaus.creditnote_pay.receiver_id=gutaus.creditnote_chosen.publisher_id;
				gutaus.member_chosen.get_member_info_of_id(gutaus.creditnote_chosen.publisher_id);
			});	
									
// known Member ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++						
			$("#btn-creditnotes-pay_to_member-known").on("click", function(){
				gutaus.members.chosen='known';
				gutaus.creditnote_pay.type_of_transfer='known member';
...
// Email ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++			
					$("#btn-creditnotes-pay_to_email").on("click", function(){
						...
						gutaus.creditnote_pay.type_of_transfer='email';
...
// Mobile +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++						
					$("#btn-creditnotes-pay_to_mobile").on("click", function(){
						...
						gutaus.creditnote_pay.type_of_transfer='mobile';
...
//*************************************************************************************************************************************************
		$(document).on("pagebeforeshow", "#pay-amount", function(){ 
			...
			$("#input-pay-amount").change( function(){   
					$(".massage-line-header").html('bezahle '+Math.abs($("#input-pay-amount").val())+' '+gutaus.creditnote_chosen.name+' an '+gutaus.member_chosen.receiver);
			});	
		...
		$(document).on("pagebeforeshow", "#pay-purpose", function(){ 
			...
			$(".gutaus-btn-next").show();
			$(".massage-line-header").html('bezahle '
										+gutaus.creditnote_pay.amount
										+' '
										+gutaus.creditnote_chosen.name
										+' an '+gutaus.member_chosen.receiver
										+' f&uuml;r '
...
//*************************************************************************************************************************************************	
					...
					$(document).on("pageshow", "#members-list", function() {
						//$("#list-members:visible").listview("refresh");
						gutaus.creditnote_pay.receiver_id="";
						$("#members-filter").blur();
						...
						$("#members-filter").keydown(function(e){
							if (e.which==13){ // Enter Key
								if (gutaus.members.chosen=='searched') {
									gutaus.members.get();
									$("#members-filter").blur();
